function[Uk_,gk_,wk_] = FlutterKMethod(~,Nm,p,K,M,A0,A1c,A1nc)
% k-method
...
for i = 1:length(invk_)
    ...
    Meff = M + c^2/(16*k^2)*(C(k)*A0 - 1i*k*(C(k)*A1c - A1nc));
    
    % Solve the eigenvalues problem
    [Vk,Dk] = eig(Meff,K); % We use the opposite (usually it is eigs(K,M) but now it the opposite: see equations of page T2.2-8 --> we have the eigenvalue (lambda) multiplying the K matrix instead of the Meff matrix)
...
disp(["The flutter speed for the k method is: ", num2str(Ufk), "m/s at a frequency", num2str(wfk/(2*pi))," Hz "])
